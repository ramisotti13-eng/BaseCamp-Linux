...
- **Widget plugins can be stopped and started again, which they now are far more often.** A page's widget services are brought in line whenever a key changes rather than only on a page switch, which is what makes #97 work, and that turned two long-standing shapes into real faults. The Video plugin never reset its stop, so once a page switch had stopped it, it stayed dead for the rest of the session. The others cleared one shared event, which let the outgoing thread carry on beside its successor: two of them drawing and pushing the same keys. Each thread carries its own stop now, in all of them. Update from the Plugins screen: Clock 1.2.4, System Monitor 1.3.3, Philips Hue 1.2, Video 1.0.3, Pipe Text 1.2.1, Snippets 1.1.

- **Snippet keys have a picture, and `{cursor}` finds its way back (plugins #15).** @FransM: a snippet key showed nothing at all, on the pad or in the editor, so a page of them was a page of keys you had to remember. Each one now shows its slot number, its name and the beginning of its text. And `{cursor}` walked back with Left presses only, which works while the way back stays on one line; most editors do not wrap a Left at the start of a line round to the end of the one above, so in vi a marker with lines under it left the cursor at the start of the last line instead. With a line to climb it now goes Home, Up per line, Home again, then Right to the column.

- **Philips Hue says why it is not connected (plugins #16).** @FransM found a stored key the bridge no longer knew, and all he was told was "Not connected", in a window with nothing to do but close it. A V1 bridge refuses with HTTP 200 and a list of error objects, and every caller here only checked whether it had a dictionary, so a forgotten key and a bridge that is switched off looked exactly the same. The bridge's own reason is shown now, the window offers to try again, All On sits beside All Off, and neither of them leaves the rows claiming something the bridge never did. Entertainment groups are left out of the list: they belong to the sync software, do not appear in the Hue app, and held the same lights under a name that came from nowhere. The value formats for the three Hue actions are written out in the plugin's help.
